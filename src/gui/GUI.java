...
public abstract class GUI {
	...
	protected abstract void rotateX(JSlider source);

	protected abstract void rotateY(JSlider source);

	protected abstract void rotateZ(JSlider source);

	protected abstract void setLightDirection(JSlider source);
	...
	public int[] getExternalLight() {
		return new int[] { externalRed.getValue(), externalGreen.getValue(),
				externalBlue.getValue() };
	}
	...
	private final JSlider externalRed = new JSlider(JSlider.HORIZONTAL, 0, 255,
			128);
	private final JSlider externalGreen = new JSlider(JSlider.HORIZONTAL, 0,
			255, 128);
	private final JSlider externalBlue = new JSlider(JSlider.HORIZONTAL, 0,
			255, 128);

	private final JSlider lightDir = new JSlider(JSlider.HORIZONTAL, -314, 314,
			0);

	private final JSlider xAxis = new JSlider(JSlider.HORIZONTAL, -314, 314, 0);
	private final JSlider yAxis = new JSlider(JSlider.HORIZONTAL, -314, 314, 0);
	private final JSlider zAxis = new JSlider(JSlider.HORIZONTAL, -314, 314, 0);
	...
	private void initialise() {
		...
		externalRed.setBackground(new Color(230, 50, 50));
		externalGreen.setBackground(new Color(50, 230, 50));
		externalBlue.setBackground(new Color(50, 50, 230));

		JPanel externalLightSliderparty = new JPanel();
		externalLightSliderparty.setLayout(new BoxLayout(externalLightSliderparty, BoxLayout.PAGE_AXIS));
		externalLightSliderparty
				.setBorder(BorderFactory.createTitledBorder("External Light"));

		externalLightSliderparty.add(externalRed);
		externalLightSliderparty.add(externalGreen);
		externalLightSliderparty.add(externalBlue);
		
		JPanel LightDirSliderparty = new JPanel();
		LightDirSliderparty.setLayout(new BoxLayout(LightDirSliderparty, BoxLayout.PAGE_AXIS));
		LightDirSliderparty
				.setBorder(BorderFactory.createTitledBorder("Light Rotation"));
		
		lightDir.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent e) {
				JSlider source = (JSlider) e.getSource();
				setLightDirection(source);
				redraw();
			}
		});

		LightDirSliderparty.add(lightDir);

		JPanel rotationSliderparty = new JPanel();
		rotationSliderparty.setLayout(new BoxLayout(rotationSliderparty,
				BoxLayout.PAGE_AXIS));
		rotationSliderparty.setBorder(BorderFactory
				.createTitledBorder("Rotation Axis"));
		...
		getyAxis().addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent e) {
				JSlider source = (JSlider) e.getSource();
				rotateX(source);
		...
		zAxis.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent e) {
				JSlider source = (JSlider) e.getSource();
				rotateZ(source);
		...
		rotationSliderparty.add(getxAxis());
		rotationSliderparty.add(getyAxis());
		rotationSliderparty.add(zAxis);
		...
		controls.add(Box.createRigidArea(new Dimension(0, 15)));
		controls.add(externalLightSliderparty);
		controls.add(Box.createRigidArea(new Dimension(0, 15)));
		controls.add(LightDirSliderparty);
		controls.add(Box.createRigidArea(new Dimension(0, 15)));
		controls.add(rotationSliderparty);
	...
	public void addxAxis(int i) {
		getxAxis().setValue(getxAxis().getValue() + i);
	}

	public void addyAxis(int i) {
		getyAxis().setValue(getyAxis().getValue() + i);
	}
	...
	public void resetSliders(){
		...
		setzAxis(0);
		lightDir.setValue(0);
		externalRed.setValue(128);
		externalGreen.setValue(128);
		externalBlue.setValue(128);
		red.setValue(128);
		green.setValue(128);
		blue.setValue(128);
	}
	...
	public JSlider getxAxis() {
		return xAxis;
	}

	public JSlider getyAxis() {
		return yAxis;
	}
}
